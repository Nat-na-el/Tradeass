...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    function Counter(selector, settings) {
      this.settings = Object.assign({ digits: 5, delay: 250, direction: '' }, settings || {});
      this.DOM = {};
      this.build(selector);
    ...
    Counter.prototype = {
      build: function(selector) {
        const scopeElm = typeof selector === 'string' ? document.querySelector(selector) : selector || this.DOM.scope;
        scopeElm.innerHTML = Array(this.settings.digits + 1).join('<div><b data-value="0"></b></div>');
        this.DOM = { scope: scopeElm, digits: scopeElm.querySelectorAll('b') };
      },
      count: function(newVal) {
        const settings = this.settings;
        const digitsElms = this.DOM.digits;
        this.value = newVal || this.DOM.scope.dataset.value | 0;
        if (!this.value) return;
        let countTo = (this.value + '').split('');
        if (settings.direction === 'rtl') {
          countTo = countTo.reverse();
          digitsElms = [].slice.call(digitsElms).reverse();
        }
        digitsElms.forEach((item, i) => {
          if (+item.dataset.value !== +countTo[i] && countTo[i] >= 0) {
            setTimeout((j) => {
              const diff = Math.abs(countTo[j] - +item.dataset.value);
    ...
    const randomCount = () => {
      const counter1 = document.querySelector('.stock-counter1');
      const counter2 = document.querySelector('.stock-counter2');
      const counter3 = document.querySelector('.stock-counter3');
      if (counter1) counter1.dataset.value = Math.floor(Math.random() * 1000000) + 100000;
      if (counter2) counter2.dataset.value = Math.floor(Math.random() * 100000) + 10000;
      if (counter3) counter3.dataset.value = Math.floor(Math.random() * 10000000) + 1000000;
    };
  ...
  return (
    <div className="relative min-h-screen flex flex-col overflow-hidden bg-gradient-to-b from-indigo-50 via-white to-blue-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-800">
      {/* Animated Trading-Themed Background – lowest layer */}
      ...
      <main className="relative z-10">
        ...
        >
          ...
          <div className="max-w-6xl mx-auto relative z-10 space-y-10 md:space-y-12">
            <motion.h1 variants={fadeInUp} className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-extrabold tracking-tight text-gray-900 dark:text-white leading-none bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400">
        ...
        >
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
            ...
            <motion.div variants={fadeInUp} className="bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-2xl p-10">
              <h3 className="text-3xl font-bold mb-8 text-center text-white">Quick Contact</h3>
