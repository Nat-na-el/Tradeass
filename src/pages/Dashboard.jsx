...
export default function Dashboard({ currentAccount }) {
  ...
  const consistencyScore = useMemo(() => {
    ...
    const streakBonus = currentStreak.count * 6; // stronger weight for streaks
    const finalScore = Math.min(100, Math.round(frequencyScore * 0.6 + streakBonus * 0.4));

    return finalScore;
  }, [monthlyTrades, currentStreak, viewDate]);
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_30%,rgba(79,70,229,0.07)_0%,transparent_70%)] animate-pulse-slow"></div>
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_70%,rgba(139,92,246,0.05)_0%,transparent_80%)] animate-pulse-slower"></div>
      </div>
      ...
      <div className={`relative min-h-screen w-full p-4 sm:p-6 lg:p-8 overflow-y-auto ${isDark ? "text-gray-200 bg-gray-950" : "text-gray-900 bg-gray-50"}`}>
        ...
        {/* Rest of your dashboard (Recent Trades, Highlights, Calendar, Floating Button, Quick Analysis Modal) */}
        {/* ... paste your existing code for these sections here ... */}
