...
export default function Dashboard({ currentAccount }) {
  ...
  // ─── Fetch account metadata & trades from subcollection ─────────
  const refreshTrades = async () => {
    ...
    if (!currentAccount?.id) {
      ...
      setAccountMeta(null);
      setLoading(false);
      setError("No account selected. Please create or select one from the sidebar.");
      return;
    }
    ...
    try {
      // 1. Account metadata (starting balance, createdAt, etc.)
      const accRef = doc(db, "users", user.uid, "accounts", currentAccount.id);
      const accSnap = await getDoc(accRef);
      if (accSnap.exists()) {
        setAccountMeta(accSnap.data());
      } else {
        setError("Selected account not found in database");
      }

      // 2. Trades subcollection
      const tradesRef = collection(accRef, "trades");
  ...
  const startingBalance = accountMeta?.starting_balance ?? 10000;
  ...
  if (!currentAccount) {
    return (
      <div className={`min-h-screen w-full p-8 flex items-center justify-center ${isDark ? "bg-gray-950" : "bg-gray-50"}`}>
        <Card className="p-10 max-w-md text-center bg-white/80 dark:bg-gray-800/80 backdrop-blur-md border border-gray-200 dark:border-gray-700 shadow-2xl">
          <AlertCircle size={64} className="mx-auto text-amber-500 mb-6" />
          <h2 className="text-3xl font-bold mb-4 text-gray-900 dark:text-gray-100">
            No Account Selected
          </h2>
          <p className="text-lg text-gray-600 dark:text-gray-400 mb-8">
            Please open the sidebar on the left and create or select a trading account to continue.
          </p>
          <button
            onClick={() => window.location.reload()}
            className="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-lg font-medium shadow-lg transition-all transform hover:scale-105"
          >
            Refresh Page
          </button>
        </Card>
  ...
  return (
    <div className={`relative min-h-screen w-full p-4 sm:p-6 lg:p-8 text-gray-100 ${isDark ? "bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950" : "bg-gradient-to-br from-gray-50 via-white to-gray-100"}`}>
      ...
      {loading ? (
        ...
        <div className="text-center p-8 text-rose-400 text-xl">
          {error}
        </div>
      ) : trades.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-96 text-center">
          <AlertCircle size={80} className="text-amber-500 mb-6 opacity-80" />
          <h2 className="text-3xl font-bold mb-4">No Trades Yet</h2>
          <p className="text-xl text-gray-500 dark:text-gray-400 mb-8 max-w-lg">
            Your account is ready, but no trades have been added yet.
          </p>
          <button
            onClick={() => navigate("/trades/new")}
            className="px-10 py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xl font-medium shadow-xl transition-all transform hover:scale-105"
          >
            Add Your First Trade
          </button>
        </div>
